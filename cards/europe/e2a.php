...
$addMoments = $db->prepare("SELECT id, userID, e2 FROM europe WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE europe SET e2 = :e2 WHERE userID = $id");


  $editAccount->bindValue(':e2',$addOne);
  $editAccount->execute();
