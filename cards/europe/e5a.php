...
$addMoments = $db->prepare("SELECT id, userID, e5 FROM europe WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE europe SET e5 = :e5 WHERE userID = $id");


  $editAccount->bindValue(':e5',$addOne);
  $editAccount->execute();
