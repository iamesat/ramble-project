...
$addMoments = $db->prepare("SELECT id, userID, e8 FROM europe WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE europe SET e8 = :e8 WHERE userID = $id");


  $editAccount->bindValue(':e8',$addOne);
  $editAccount->execute();
