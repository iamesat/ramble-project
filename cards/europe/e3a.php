...
$addMoments = $db->prepare("SELECT id, userID, e3 FROM europe WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE europe SET e3 = :e3 WHERE userID = $id");


  $editAccount->bindValue(':e3',$addOne);
  $editAccount->execute();
