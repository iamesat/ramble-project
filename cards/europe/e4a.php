...
$addMoments = $db->prepare("SELECT id, userID, e4 FROM europe WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE europe SET e4 = :e4 WHERE userID = $id");


  $editAccount->bindValue(':e4',$addOne);
  $editAccount->execute();
