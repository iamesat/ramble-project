...
$addMoments = $db->prepare("SELECT id, userID, a1 FROM afrika WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE afrika SET a1 = :a1 WHERE userID = $id");


  $editAccount->bindValue(':a1',$addOne);
  $editAccount->execute();
