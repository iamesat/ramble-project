...
$addMoments = $db->prepare("SELECT id, userID, a12 FROM afrika WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE afrika SET a12 = :a12 WHERE userID = $id");


  $editAccount->bindValue(':a12',$addOne);
  $editAccount->execute();
