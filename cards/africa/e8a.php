...
$addMoments = $db->prepare("SELECT id, userID, a8 FROM afrika WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE afrika SET a8 = :a8 WHERE userID = $id");


  $editAccount->bindValue(':a8',$addOne);
  $editAccount->execute();
