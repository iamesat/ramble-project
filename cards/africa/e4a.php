...
$addMoments = $db->prepare("SELECT id, userID, a4 FROM afrika WHERE userID=$id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE afrika SET a4 = :a4 WHERE userID = $id");


  $editAccount->bindValue(':a4',$addOne);
  $editAccount->execute();
