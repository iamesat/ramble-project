...
$addMoments = $db->prepare("SELECT id, userID, s6 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s6 = :s6 WHERE userID = $id");


  $editAccount->bindValue(':s6',$addOne);
  $editAccount->execute();
