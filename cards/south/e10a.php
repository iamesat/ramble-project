...
$addMoments = $db->prepare("SELECT id, userID, s10 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s10 = :s10 WHERE userID = $id");


  $editAccount->bindValue(':s10',$addOne);
  $editAccount->execute();
