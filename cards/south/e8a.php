...
$addMoments = $db->prepare("SELECT id, userID, s8 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s8 = :s8 WHERE userID = $id");


  $editAccount->bindValue(':s8',$addOne);
  $editAccount->execute();
