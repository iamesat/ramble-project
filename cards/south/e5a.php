...
$addMoments = $db->prepare("SELECT id, userID, s5 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s5 = :s5 WHERE userID = $id");


  $editAccount->bindValue(':s5',$addOne);
  $editAccount->execute();
