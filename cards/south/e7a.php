...
$addMoments = $db->prepare("SELECT id, userID, s7 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s7 = :s7 WHERE userID = $id");


  $editAccount->bindValue(':s7',$addOne);
  $editAccount->execute();
