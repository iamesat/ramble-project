...
$addMoments = $db->prepare("SELECT id, userID, s9 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s9 = :s9 WHERE userID = $id");


  $editAccount->bindValue(':s9',$addOne);
  $editAccount->execute();
