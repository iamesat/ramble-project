...
$addMoments = $db->prepare("SELECT id, userID, s4 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s4 = :s4 WHERE userID = $id");


  $editAccount->bindValue(':s4',$addOne);
  $editAccount->execute();
