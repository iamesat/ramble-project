...
$addMoments = $db->prepare("SELECT id, userID, s11 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s11 = :s11 WHERE userID = $id");


  $editAccount->bindValue(':s11',$addOne);
  $editAccount->execute();
