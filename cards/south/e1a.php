...
$addMoments = $db->prepare("SELECT id, userID, s1 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s1 = :s1 WHERE userID = $id");


  $editAccount->bindValue(':s1',$addOne);
  $editAccount->execute();
