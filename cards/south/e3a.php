...
$addMoments = $db->prepare("SELECT id, userID, s3 FROM zuidamerika WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE zuidamerika SET s3 = :s3 WHERE userID = $id");


  $editAccount->bindValue(':s3',$addOne);
  $editAccount->execute();
