...
$addMoments = $db->prepare("SELECT id, userID, c2 FROM australie WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE australie SET c2 = :c2 WHERE userID = $id");


  $editAccount->bindValue(':c2',$addOne);
  $editAccount->execute();
