...
$addMoments = $db->prepare("SELECT id, userID, c5 FROM australie WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE australie SET c5 = :c5 WHERE userID = $id");


  $editAccount->bindValue(':c5',$addOne);
  $editAccount->execute();
