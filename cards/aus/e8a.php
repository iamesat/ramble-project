...
$addMoments = $db->prepare("SELECT id, userID, c8 FROM australie WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE australie SET c8 = :c8 WHERE userID = $id");


  $editAccount->bindValue(':c8',$addOne);
  $editAccount->execute();
