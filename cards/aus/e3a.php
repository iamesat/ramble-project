...
$addMoments = $db->prepare("SELECT id, userID, c3 FROM australie WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE australie SET c3 = :c3 WHERE userID = $id");


  $editAccount->bindValue(':c3',$addOne);
  $editAccount->execute();
