...
$addMoments = $db->prepare("SELECT id, userID, c4 FROM australie WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE australie SET c4 = :c4 WHERE userID = $id");


  $editAccount->bindValue(':c4',$addOne);
  $editAccount->execute();
