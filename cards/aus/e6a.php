...
$addMoments = $db->prepare("SELECT id, userID, c6 FROM australie WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE australie SET c6 = :c6 WHERE userID = $id");


  $editAccount->bindValue(':c6',$addOne);
  $editAccount->execute();
