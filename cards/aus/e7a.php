...
$addMoments = $db->prepare("SELECT id, userID, c7 FROM australie WHERE userID = $id");
$addMoments->execute();
...
if (isset($_POST)) {
  ...
  $editAccount = $db->prepare("UPDATE australie SET c7 = :c7 WHERE userID = $id");


  $editAccount->bindValue(':c7',$addOne);
  $editAccount->execute();
